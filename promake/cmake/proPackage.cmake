message(STATUS "include proPackage.cmake")

set(pro_${PROJECT_NAME}_have_dependencies FALSE)
function(pro_DecodeVerson major minor patch version)
  if("${version}" MATCHES "^([0-9]+)\\.([0-9]+)\\.([0-9]+)")
    set(${major} ${CMAKE_MATCH_1} PARENT_SCOPE)
    set(${minor} ${CMAKE_MATCH_2} PARENT_SCOPE)
    set(${patch} ${CMAKE_MATCH_3} PARENT_SCOPE)
    elif("${version}" MATCHES "^([0-9]+)\\.([0-9]+)")
    set(${major} ${CMAKE_MATCH_1} PARENT_SCOPE)
    set(${minor} ${CMAKE_MATCH_2} PARENT_SCOPE)
...
function(pro_ToPackageName rst name version)
...
macro(pro_AddDepPro projectName name version)
  set(pro_${projectName}_have_dependencies TRUE)
  list(FIND pro_${projectName}_dep_name_list "${name}" _idx)
  if(_idx EQUAL -1)
  ...
  if(_need_find)
    list(APPEND pro_${projectName}_dep_name_list ${name})
    list(APPEND pro_${projectName}_dep_version_list ${version})
    message(STATUS "find package: ${name} ${version}")
    find_package(${name} ${version} QUIET)
    if(${${name}_FOUND})
      message(STATUS "${name} ${${name}_VERSION} found")
    else()
      set(_address "https://github.com/pro/${name}")
...
macro(pro_AddDep name version)
  pro_AddDepPro(${PROJECT_NAME} ${name} ${version})
endmacro()
...
include_directories(\"\${include_dir}\")\n")
  ...
  if(ARG_CPM OR pro_${PROJECT_NAME}_have_dependencies)
...
if(NOT UCMake_FOUND)
  ...
  if(NOT UCMake_FOUND)
    set(_pro_UCMake_address \"https://github.com/pro/UCMake\")
    message(STATUS \"UCMake ${UCMake_VERSION} not found\")
    message(STATUS \"fetch: \${_pro_UCMake_address} with tag \${UCMake_VERSION}\")
    FetchContent_Declare(
      UCMake
      GIT_REPOSITORY \${_pro_UCMake_address}
...
"
    ...
    list(LENGTH pro_${PROJECT_NAME}_dep_name_list _dep_num)
    if(_dep_num GREATER 0)
      message(STATUS "[Dependencies]")
      list(LENGTH pro_${PROJECT_NAME}_dep_name_list _dep_num)
      math(EXPR _stop "${_dep_num}-1")
      foreach(index RANGE ${_stop})
        list(GET pro_${PROJECT_NAME}_dep_name_list ${index} dep_name)
        list(GET pro_${PROJECT_NAME}_dep_version_list ${index} dep_version)
        message(STATUS "- ${dep_name} ${dep_version}")
        string(APPEND UBPA_PACKAGE_INIT "pro_AddDepPro(${PROJECT_NAME} ${dep_name} ${dep_version})\n")
      endforeach()
    endif()
  endif()
