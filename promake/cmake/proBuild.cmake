message(STATUS "include proBuild.cmake")

function(pro_AddSubDirsRec path)
...
function(pro_GetTargetName rst targetPath)
...
function(_pro_ExpandSources rst _sources)
  set(tmp_rst "")
...
function(pro_AddTarget)
  ...
  # test
  if(ARG_TEST AND NOT ${pro_BuildTest_${PROJECT_NAME}})
    return()
  endif()
  ...
  _pro_ExpandSources(sources_public ARG_SOURCE_PUBLIC)
  _pro_ExpandSources(sources_interface ARG_SOURCE_INTERFACE)
  _pro_ExpandSources(sources_private ARG_SOURCE)
  ...
  pro_GetTargetName(coreTargetName ${CMAKE_CURRENT_SOURCE_DIR})
  ...
  if("${ARG_MODE}" STREQUAL "EXE")
    ...
    add_executable("pro::${coreTargetName}" ALIAS ${coreTargetName})
    if(MSVC)
  ...
  elseif("${ARG_MODE}" STREQUAL "STATIC")
    ...
    add_library("pro::${coreTargetName}" ALIAS ${coreTargetName})
    list(APPEND targetNames ${coreTargetName})
  ...
  elseif("${ARG_MODE}" STREQUAL "STATIC_AND_SHARED")
    ...
    add_library("pro::${coreTargetName}_static" ALIAS ${coreTargetName}_static)
    add_library(${coreTargetName}_shared SHARED)
    add_library("pro::${coreTargetName}_shared" ALIAS ${coreTargetName}_shared)
...
COMPILE_PDB_NAME_RELEASE ${targetName}${CMAKE_DEBUG_POSTFIX}
COMPILE_PDB_NAME_MINSIZEREL ${targetName}${CMAKE_MINSIZEREL_POSTFIX}
COMPILE_PDB_NAME_RELWITHDEBINFO ${targetName}${CMAKE_RELWITHDEGINFO_POSTFIX}
)
    endif()
...
LIBRARY DESTINATION "${package_name}/lib"
      ...
      if(${ARG_MODE} STREQUAL "STATIC" OR ${ARG_MODE} STREQUAL "SHARED" OR ${ARG_MODE} STRQUAL "STATIC_AND_SHARED")
        # dll
        install(FILES "${CMAKE_LIBRARY_OUTPUT_DIRECTORY_DEBUG}/${targetName}${CMAKE_DEBUG_POSTFIX}.pdb"
...
CONFIGURATIONS Release DESTINATION "bin" OPTIONAL
        ...
        install(FILES "${CMAKE_LIBRARY_OUTPUT_DIRECTORY_MINSIZEREL}/${targetName}${CMAKE_MINSIZEREL_POSTFIX}.pdb"
CONFIGURATIONS MinSizeRel DESTINATION "bin" OPTIONAL
)
        install(FILES "${CMAKE_LIBRARY_OUTPUT_DIRECTORY_RELWITHDEBINFO}/${targetName}${CMAKE_RELWITHDEBINFO_POSTFIX}.pdb"
CONFIGURATIONS RelWithDebInfo DESTINATION "bin" OPTIONAL
)

        # lib
        install(FILES "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_DEBUG}/${targetName}${CMAKE_DEBUG_POSTFIX}.pdb"
CONFIGURATIONS Debug DESTINATION "${package_name}/lib" OPTIONAL
)
        install(FILES "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELEASE}/${targetName}${CMAKE_RELEASE_POSTFIX}.pdb"
CONFIGURATIONS Release DESTINATION "${package_name}/lib" OPTIONAL
)
        install(FILES "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_MINSIZEREL}/${targetName}${CMAKE_MINSIZEREL_POSTFIX}.pdb"
CONFIGURATIONS MinSizeRel DESTINATION "${package_name}/lib" OPTIONAL
)

        install(FILES "${CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELWITHDEBINFO}/${targetName}${CMAKE_RELWITHDEBINFO_POSTFIX}.pdb"
