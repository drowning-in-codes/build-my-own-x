...
std::string COURSES_OFFERED_CSV_PATH =
    "E:/build-my-own-x/cs106L/simpleenroll_starter/student_output/"
    "courses_offered.csv";

std::string COURSES_NOT_OFFERED_CSV_PATH =
    "E:/build-my-own-x/cs106L/simpleenroll_starter/student_output/"
    "courses_not_offered.csv";
...
void parse_csv(std::string filename, std::vector<Course> &vector_of_courses) {
  // STUDENT TODO: Implement this spaceString function
  ...
  while (std::getline(file, line)) {
    ...
    if (end_pos != std::string::npos) {
      std::string col = line.substr(start, end_pos - start);
      c.title = col;
    } else {
      throw std::runtime_error("columns not match");
    }

    start = end_pos + 1;
    end_pos = line.find(',', start);
    if (end_pos != std::string::npos) {
      std::string units = line.substr(start, end_pos - start);
      c.number_of_units = units;
    } else {
      throw std::runtime_error("columns not match");
    }
    start = end_pos + 1;
    std::string quarter = line.substr(start);
    c.quarter = quarter;
...
void write_courses_offered(std::vector<Course> &vector_of_courses) {
  ...
  std::ofstream output_file(COURSES_OFFERED_CSV_PATH,
                            std::ios::out | std::ios::trunc);

  if (!output_file.is_open()) {
    throw std::runtime_error("File not found");
  }
  std::vector<Course> deleteCourse;
  // output_file << "Title,Number of Units,Quarter\n";
  for (int i = 0; i < vector_of_courses.size(); i++) {
    auto &course = vector_of_courses.at(i);
    if (course.quarter == "null") {
      continue;
    }
    output_file << course.title << "," << course.number_of_units << ","
                << course.quarter;
    // if (i != vector_of_courses.size() - 1) {
    output_file << "\n";
    // }
    deleteCourse.push_back(course);
  }

  output_file.close();
  for (auto &c : deleteCourse) {
    delete_elem_from_vector(vector_of_courses, c);
  }
}
...
void write_courses_not_offered(std::vector<Course> &vector_of_courses) {
  // STUDENT TODO: implement this function
  std::ofstream output_file(COURSES_NOT_OFFERED_CSV_PATH,
                            std::ios::out | std::ios::trunc);
  if (!output_file.is_open()) {
    throw std::runtime_error("File not found");
  }
...
void delete_elem_from_vector(std::vector<Course> &v, Course &elem) {
  // cause override the operator==, find works
